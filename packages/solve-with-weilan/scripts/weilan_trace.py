...
def valid_conversation_source(value):
    ...
    return "#" in locator and all(part.strip() for part in locator.split("#", 1))
...
def active_semantic_entries(entries):
    superseded = {
        memory_id
...
def build_semantic_index_value(workspace, scope, warnings=None):
    ...
    entries = active_semantic_entries(all_entries)
    indexed_entries = {}
    terms = {}
    for entry in entries:
...
def semantic_index_is_fresh(index, workspace, scope):
    ...
    return not warnings and index.get("evidence_dependency_state", {}) == semantic_evidence_dependency_state(
        entries
    )
...
def command_evidence_capture(args):
    ...
    if not any(valid_conversation_source(source) for source in sources):
        raise ValueError(
            "conversation evidence requires --source conversation:<thread-or-session>#<turn-or-range>"
        )
...
def assert_governance_write_allowed(workspace, scope):
    ...
    if not scoped_control or scoped_control.get("state") != "active":
        raise ValueError("governance writes require an explicitly active scope")
