...
A projection contains only current focus, status, next proposed action, compact decisions, open questions, source pointers, source snapshots, and the control heads from which it was derived.
...
python scripts/weilan_trace.py evidence-capture `
  ...
  --source "conversation:<thread-or-session>#<turn-or-range>"
```
...
The claim is a paraphrased evidence fragment, not a transcript. It is limited to six lines and 4 KiB including metadata. Credential-like content and private-key material are rejected without persistence. A valid `conversation:...#...` source is mandatory; frame and file sources may supplement it but cannot replace it.
