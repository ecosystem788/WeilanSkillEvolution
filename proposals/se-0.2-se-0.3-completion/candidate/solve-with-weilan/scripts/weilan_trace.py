...
def conversation_turn_snapshot(source):
    ...
    candidates = sorted(root.rglob(f"*{thread_id}*.jsonl")) if root.exists() else []
    matches = []
    for path in candidates:
        ...
        try:
            ...
            continue
        if session_id == thread_id and messages:
            matches.append((path, messages))
    if len(matches) != 1:
