...
SEMANTIC_PENDING_SCHEMA_VERSION = "weilan_semantic_pending_probe_v0.1"
...
def semantic_pending_directory(workspace, scope):
    return (
        state_root()
        / "memory"
        / "semantic-pending-probe"
        / "workspaces"
        / workspace_key(workspace)
        / scope_key(scope)
    )
...
def load_semantic_pending_records(workspace, scope, warnings=None):
    warnings = warnings if warnings is not None else []
    records = []
    root = semantic_pending_directory(workspace, scope)
    if not root.exists():
        return records
    for path in sorted(root.glob("*.jsonl")):
        sink = []
        for record in read_jsonl_records(path, sink):
            if record.get("schema_version") != SEMANTIC_PENDING_SCHEMA_VERSION:
                sink.append(f"{path}: unsupported semantic pending schema")
                continue
            if normalized_workspace(record.get("workspace", "")) != normalized_workspace(workspace):
                sink.append(f"{path}: workspace mismatch")
                continue
            if normalize_scope(record.get("scope")).casefold() != normalize_scope(scope).casefold():
                sink.append(f"{path}: scope mismatch")
                continue
            if record.get("event_type") not in {"pending_captured", "pending_admitted"}:
                sink.append(f"{path}: unknown semantic pending event_type")
                continue
            records.append(record)
...
def queued_semantic_pending(records):
    admitted = {
        record.get("pending_id")
        for record in records
        if record.get("event_type") == "pending_admitted"
    }
    queued = []
    seen = set()
    for index, record in enumerate(records):
        pending_id = record.get("pending_id")
        if record.get("event_type") != "pending_captured":
            continue
        if not pending_id or pending_id in admitted or pending_id in seen:
            continue
        seen.add(pending_id)
        item = dict(record)
        item["queue_index"] = index
        queued.append(item)
    return sorted(queued, key=lambda item: (item["queue_index"], item["pending_id"]))


def append_semantic_pending_event(workspace, scope, event):
    path = semantic_pending_directory(workspace, scope) / f"{datetime.now(timezone.utc):%Y-%m-%d}.jsonl"
    append_event(path, event)
    return path
...
def semantic_candidate_payload(workspace, scope, kind, summary, detail, tags, sources, supersedes, conflicts_with):
    if not summary.strip():
        raise ValueError("semantic summary cannot be empty")
    warnings = []
    existing = load_semantic_entries(workspace, scope, warnings)
    if warnings:
        raise ValueError("semantic ledger is invalid: " + "; ".join(warnings))
...
def capture_budget_denied_pending(
    ...
    payload = semantic_candidate_payload(
        workspace, scope, kind, summary, detail, tags, sources, supersedes, conflicts_with
    )
    warnings = []
    try:
        enforce_semantic_budget(
            workspace, scope, 1, [], leaving_ids=payload["supersedes"], warnings=warnings
        )
    except ValueError as exc:
        if not str(exc).startswith("semantic budget exhausted:"):
            raise
        denial = str(exc)
    else:
        raise ValueError("semantic budget was not exhausted; pending capture would mint existence")

    records = load_semantic_pending_records(workspace, scope, warnings)
    if warnings:
        raise ValueError("semantic pending ledger is invalid: " + "; ".join(warnings))
    existing_pending_ids = {record.get("pending_id") for record in records}
    if pending_id:
        try:
            pending_id = str(uuid.UUID(pending_id))
        except ValueError as exc:
            raise ValueError("--pending-id must be a UUID") from exc
    else:
        pending_id = str(uuid.uuid4())
    if pending_id in existing_pending_ids:
        raise ValueError(f"pending id already exists: {pending_id}")

    event = {
        "schema_version": SEMANTIC_PENDING_SCHEMA_VERSION,
        "event_type": "pending_captured",
        "event_id": str(uuid.uuid4()),
        "pending_id": pending_id,
        "timestamp_utc": utc_now(),
        "authority": "probe_private_pending_never_recall_authority",
        "workspace": workspace,
        "workspace_key": workspace_key(workspace),
        "scope": scope,
        "scope_key": scope_key(scope),
        "budget_denial": denial,
        "candidate": payload,
    }
    path = append_semantic_pending_event(workspace, scope, event)
    return {
        "captured": True,
        "pending_id": pending_id,
        "workspace": workspace,
...
def settle_one_pending_consumer(workspace, scope, release_event_id):
    warnings = []
    queue = queued_semantic_pending(load_semantic_pending_records(workspace, scope, warnings))
    if warnings:
        raise ValueError("semantic pending ledger is invalid: " + "; ".join(warnings))
    if not queue:
        return {"settled": False, "reason": "no_pending_consumer"}
    consumer = queue[0]
    ...
    event = {
        "schema_version": SEMANTIC_PENDING_SCHEMA_VERSION,
        "event_type": "pending_admitted",
        "event_id": str(uuid.uuid4()),
        "pending_id": consumer["pending_id"],
        "memory_id": entry["memory_id"],
        "timestamp_utc": utc_now(),
        "authority": "probe_release_settlement_never_activation_authority",
        "workspace": workspace,
        "workspace_key": workspace_key(workspace),
        "scope": scope,
        "scope_key": scope_key(scope),
...
def command_memory_pending_capture(args):
    prohibited = [
        source for source in args.source
        if has_protected_grounding_prefix(source)
    ]
    ...
    with contract_fence(
        ...
        result = capture_budget_denied_pending(
            workspace,
            scope,
            args.kind,
            args.summary,
...
def command_memory_pending_show(args):
    workspace = canonical_workspace(args.workspace)
    scope = normalize_scope(args.scope)
    warnings = []
    records = load_semantic_pending_records(workspace, scope, warnings)
    queued = queued_semantic_pending(records)
    admitted = [
        {
            "pending_id": record.get("pending_id"),
            "memory_id": record.get("memory_id"),
            "release_event_id": record.get("release_event_id"),
        }
        for record in records
        if record.get("event_type") == "pending_admitted"
    ]
        ...
        "queued": [
            {
                ...
                "kind": record.get("candidate", {}).get("kind"),
                "summary": record.get("candidate", {}).get("summary", ""),
...
def command_memory_disposition_fenced(args, workspace, scope):
    ...
    if getattr(args, "settle_pending", False) and args.state == "active":
        raise ValueError("--settle-pending requires a releasing dormant or retired disposition")
    displaced = []
    ...
    append_event(path, event)
    settlement = None
    if getattr(args, "settle_pending", False):
        settlement = settle_one_pending_consumer(workspace, scope, event["event_id"])
    if displaced:
    ...
    if settlement is not None:
        result["pending_settlement"] = settlement
    print(json.dumps(result, ensure_ascii=False, indent=2))
...
def build_parser():
    ...
    memory_pending_capture_parser = subparsers.add_parser(
        "memory-pending-capture",
        help="probe-only: queue a valid semantic entry that was denied solely by active-slot budget",
    )
    memory_pending_capture_parser.add_argument("--workspace", default=os.getcwd())
    memory_pending_capture_parser.add_argument("--scope", default="workspace")
    memory_pending_capture_parser.add_argument("--kind", choices=SEMANTIC_KINDS, required=True)
    memory_pending_capture_parser.add_argument("--summary", required=True)
    memory_pending_capture_parser.add_argument("--detail", default="")
    memory_pending_capture_parser.add_argument("--tag", action="append", default=[])
    memory_pending_capture_parser.add_argument("--source", action="append", required=True)
    memory_pending_capture_parser.add_argument("--supersedes", action="append", default=[])
    memory_pending_capture_parser.add_argument("--conflicts-with", action="append", default=[])
    memory_pending_capture_parser.add_argument("--pending-id", help="optional deterministic UUID for paired probe arms")
    memory_pending_capture_parser.set_defaults(func=command_memory_pending_capture)

    memory_pending_show_parser = subparsers.add_parser(
        "memory-pending-show",
        help="probe-only: show queued/admitted semantic pending entries without exposing them to recall",
    )
    memory_pending_show_parser.add_argument("--workspace", default=os.getcwd())
    memory_pending_show_parser.add_argument("--scope", required=True)
    memory_pending_show_parser.set_defaults(func=command_memory_pending_show)
    ...
    memory_disposition_parser.add_argument("--settle-pending", action="store_true", help="probe-only: admit one queued budget-denied pending entry when this disposition releases a slot")
