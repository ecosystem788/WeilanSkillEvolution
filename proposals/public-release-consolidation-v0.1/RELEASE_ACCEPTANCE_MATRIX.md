...
| R16 Publication | Push/tag/release uses the signed RC and publishes hashes/rollback receipt | Annotated tag `weilan-windows-first-release-rc5` points to the exact signed RC5 commit `049d6c79e683b829bd8fa0b9a5491f5a3796a0bb` (Git tree `b159368309de399587f5d768d6680674ee54277b`), whose fresh detached hygiene is `PASS/68/0/0` at tree `fa917918d3598a1063f851322e4a6bc22fe32f41a2966fa2dd608766cbe56013`; `R16_PUBLICATION_RECEIPT.json` publishes the hashes, R15 v4 PASS reference, governance signatures, accepted T2 boundary, and executable rollback commands | PASS — RELEASED (Windows first release; T2 remains an accepted open/delegated boundary) | Owner authorization 2026-07-19 10:41:26; R16 double-sign: proposal 10:46:20 + agreement 10:54:41. Any tag-target drift or candidate-byte drift invalidates R16; rollback cannot recall a tag already fetched by a third party |
...
`RELEASED` — Windows first release, with the R12 T2 clean-machine result
explicitly retained as an accepted open/delegated boundary.
...
`f92338b`). R16 is now closed PASS by the annotated
`weilan-windows-first-release-rc5` tag and
`R16_PUBLICATION_RECEIPT.json`, under owner authorization 2026-07-19 10:41:26
and community double-sign 10:46:20/10:54:41. The release verdict is therefore
`RELEASED`; T2 remains visibly unverified, open/delegated, and nonblocking.
Signatures do not survive candidate-byte drift.
...
2. T2 is accepted as a delegated boundary; the independent exact-commit final
   ...
   committed `f92338b`).
3. R16 publication is complete: preserve the exact RC5 tag target and the
   published rollback receipt. Any candidate-byte or tag-target drift reopens
   the gate; deleting the remote tag cannot recall copies already fetched by a
   third party.
