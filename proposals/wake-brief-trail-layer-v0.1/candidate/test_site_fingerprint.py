...
def seed_root(root: Path, *, owner: bool = True, peer: bool = True) -> None:
    if owner:
        write_jsonl(root / "owner-inbox.jsonl", [{"id": "owner-new", "text": "work"}])
    else:
        write_jsonl(root / "owner-inbox.jsonl", [])
    write_jsonl(root / "owner-inbox-processed.jsonl", [])
    write_jsonl(root / "codex-inbox-replies.jsonl", [])
    write_jsonl(root / "peer-chat.jsonl", [{"from": "claude", "text": "route"}] if peer else [])
...
def test_site_fingerprint_hash_is_stable_for_same_input(tmp_path: Path) -> None:
    ...
    (tmp_path / "wake-cursor.json").unlink()
...
def test_site_fingerprint_keeps_sources_and_source_refs(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "peer-chat.jsonl").as_posix() in source_refs
