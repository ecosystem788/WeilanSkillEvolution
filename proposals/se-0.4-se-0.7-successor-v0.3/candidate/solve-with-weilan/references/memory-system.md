...
For milestone work with hard tool budgets, the expected continuity path is one recall, one lineage inspection, one lineaged open, one focused implementation, one focused verification, and one close/validate. Skip archival, retention planning, semantic searches, self projection, and governance inspection unless the task explicitly requires them or the activation gate blocks progress.
