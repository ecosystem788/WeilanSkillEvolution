...
Before an artifact, log, memory projection, receipt, or prior result supports a verdict or next action, run an evidence-freshness gate. Check whether the source is current for the requested scope: compare available timestamps, source hashes, control heads, draft-vs-official status, and any superseding artifact or receipt named by the workspace authority. If the source is stale, ambiguous, or superseded, do not use it as decisive support until it is refreshed or explicitly marked as stale evidence. A stale-evidence correction is a recordable gate firing when it changes the recorded first intent.

Before emitting a verdict, adoption recommendation, deployment action, rollback action, or other consequential next step, run an authority-boundary gate. Classify whether the step is already authorized by user instruction, Control Ledger state, and the external authority plane for the workspace. If the step requires owner authority that is not present, stop at a bounded recommendation or request for authorization. A prevented overstep is a recordable gate firing when it changes the recorded first intent.
