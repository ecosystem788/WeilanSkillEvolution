...
## Record L2/L3 first intent

For L2/L3 tasks, before running verification, freshness, authority, or governance gates that could change the outcome, record a first-intent snapshot. The snapshot states the intended verdict, attribution, or next action and the exact evidence basis available at that moment. Keep it observable in the task artifact or Frame/Trace record; do not record private reasoning.

If a later gate changes that intent, the final receipt or `method_impact_trace` must cite the first-intent snapshot as the counterfactual, name the gate that fired, state the observable failure avoided, exposed, or left unchanged, and report added tool calls, elapsed milliseconds, and context-token cost when those costs are available. If no gate changes the recorded intent, do not claim method impact from the snapshot.
