...
- **L3:** restore scope once; inspect lineage once; continue one real Frame; implement the current milestone; run the milestone verification; close the Frame. In a three-stage milestone task with a 40-call aggregate budget, keep each stage to 12 task calls or fewer when possible. Do not search broad history, rebuild projections, or repeat reference reads unless activation is `STALE`, a command fails, or the task evidence contradicts the current holder.
