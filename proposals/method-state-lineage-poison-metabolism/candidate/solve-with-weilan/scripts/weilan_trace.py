...
def validate_frame_event_for_append(event, index):
    errors = []
...
def append_frame_event(path, event, existing_events=None):
    index = len(existing_events or []) + 1
    errors = validate_frame_event_for_append(event, index)
    if errors:
        raise ValueError("invalid frame event: " + "; ".join(errors))
    append_event(path, event)


def write_frame_event_file_atomic(path, event):
    errors = validate_events([event])
    if errors:
        raise ValueError("invalid frame event: " + "; ".join(errors))
    write_event_file_atomic(path, event)
...
def command_open_lineaged_fenced(args, workspace, scope):
    ...
    with exclusive_file_lock(lock_path):
        ...
        try:
            write_frame_event_file_atomic(frame_path, frame_event)
...
def command_event_fenced(args, path):
    ...
    append_frame_event(path, event, existing_events=events)
