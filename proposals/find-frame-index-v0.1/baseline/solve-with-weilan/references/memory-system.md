...
### Source-authenticity interpretation marker

Every newly written promotion from either `memory-note` or `evidence-promote` carries a derived `source_authenticity` marker. The marker reports only facts supported by the existing capture contract:

- `testimonial_attestation.present` is true when provenance contains a resolved `conversation:` source. This means testimony is present and needs in-person review; it does not mean the testimony is true or sufficient for the claim.
- `non_testimonial_anchors` lists recognizable file, Frame, evidence, and semantic-memory source anchors. An anchor is a locator, not proof that a machine check ran.
- `observational_derivation.present` is always false with reason `current_capture_contract_has_no_machine-check-proof`. Neither current public promotion entrance carries a non-self-asserted machine-check proof contract.
- `conclusion` is always `no_authenticity_conclusion`.

The marker is derived from the evidence provenance and promotion summary binding. Callers cannot supply a classification, basis, truth flag, or observational result. In particular, a promotion with both conversation and file/Frame sources deterministically reports testimony plus non-testimonial anchors; it never silently chooses one class and never treats the anchor as an observation.

Historical promotions without the marker remain readable as `unclassified`, `legacy`, and `no_authenticity_conclusion`; they are not rewritten. A malformed marker, missing provenance, changed summary binding, or provenance mismatch is exposed as `untrusted` and never upgraded to a truth conclusion. This marker is explanatory only: failure to prove authenticity does not reject an otherwise valid promotion under the existing Promotion Gate.
