...
def event_matches(goal, observed):
    if goal.get("state") != "ACTIVE":
        return False
    registered_event_id = observed.get("registered_event_id")
    if not registered_event_id or registered_event_id != goal.get("registered_event_id"):
        return False
...
def legacy_committed_event_matches(goal, observed, transition_sequence):
    """Validate one pre-binding observed event only for committed replay."""
    if observed.get("registered_event_id"):
        return False
    registered_sequence = goal.get("registered_sequence")
    observed_sequence = observed.get("sequence")
    if not (
        isinstance(registered_sequence, int)
        and isinstance(observed_sequence, int)
        and isinstance(transition_sequence, int)
        and registered_sequence < observed_sequence < transition_sequence
    ):
        return False
    condition = goal.get("condition", {})
    if condition.get("event_kind") != observed.get("event_kind"):
    ...
    for index, record in enumerate(records, 1):
        ...
        if event_type == "goal_registered":
            ...
            if target_state == "SATISFIED":
                ...
                matches = observed and (
                    event_matches(goal, observed)
                    or legacy_committed_event_matches(goal, observed, sequence)
                )
                if not matches:
