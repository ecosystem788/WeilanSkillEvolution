...
# Process-local id -> [path] table for find_frame. Purely derived: never written to
...
def _frame_index(frames_root):
    ...
    for date_entry in date_dirs:
        ...
        for entry in entries:
            name = entry.name
            if not name.endswith(".jsonl") or not entry.is_file():
                continue
...
def find_frame(frame_id):
    frames_root = state_root() / "frames"
    candidates = [
        path for path in _frame_index(frames_root).get(frame_id, ()) if path.exists()
    ]
    if not candidates:
        # (a) nothing indexed, or every indexed path has since vanished: the table
        # cannot answer this id, so fall back to the authoritative glob.
...
def note_frame_write(path):
    """Obligation (b): a write under frames/ invalidates the process-local table.

    Only a write below the frames root the table was built from can change the
    id -> path mapping, so anything else is left alone and keeps its speedup.
    """
    if _FRAME_INDEX_ROOT is None:
        return
    if str(Path(path).parent.parent) == _FRAME_INDEX_ROOT:
        invalidate_frame_index()


def append_event(path, event):
    note_frame_write(path)
