...
## 九、边界 2 的前半句被兑现：锁内时间线是实测的了（2026-08-02，n=3，复制账本）

第七节边界 2 说：「'advisory 占持锁窗 96%' 是静态+剖面得到的，把两者接起来是推断，不是实测」。
本节把**前半句**兑现——不再靠剖面接静态，而是从进程外部直接量锁内时间线。
**后半句（07-22/07-24 那几次真实超时当时谁持锁）仍未测，且不可追补**，理由见本节末。

### 9.1 方法：不改机件，只在进程外看四个可观测点

`command_open_lineaged` 从 `:538` 进 `contract_fence` 到 `:688` 打印，全程罩在三把锁下。
锁内发生的事里，有两件会在**文件系统上留下时刻**，可以从外部看见：

| 观测点 | 怎么看见 | 对应源码 |
|---|---|---|
| `t_spawn` | open 子进程启动（持锁窗起点的**上界**） | — |
| `t_frame` | `frames/<date>/<frame_id>.jsonl` 出现（原子 rename） | `:667` `write_event_file_atomic` |
| `t_heads` | 血统 heads 文件 `mtime_ns` 变化 | `:677` `write_lineage_heads` |
| `t_release` | **另一个进程**阻塞在同一把 `.workspace-contract.lock` 上，终于拿到 | `:538` 的 fence 退出 |

`:677` 之后到 `:688` 之间，源码里只有三样东西：构造 `output` 字典、
`attach_trace_advisory_result`（`:685`）、`print`（`:688`）。**没有任何写。**
所以 `[t_heads, t_release]` 这一段，就是"持锁做只读咨询"的那一段，**量它 = 量 advisory**。

**争用方是真的在等**：竞争进程在 `t_spawn+1.5s` 才启动（此时帧已落盘约 1.06s），
一直阻塞到 open 释放。所以"锁在整个 advisory 窗口里被连续持有"这件事，
是这个进程**用自己被阻塞的 12–13 秒证明的**，不是推断出来的。

**不碰生产**：全程用 `WEILAN_METHOD_HOME` 指向一份复制账本
（6,560 文件 / 49 MB，frames 6,209 份，lineage 6,002 条），
且用 `relation fork` 从一个**已闭合**的头帧长出一次性分支，副本 main 头不动。
跑完核验生产账本：frames 仍 6,209 份、main 头仍 `wf-20260801-151202-c32d88`、
`probe-*` 分支 0 条；四个探针帧全落在副本里（副本 6,213）。

### 9.2 实测（丢弃 1 次预热，warm n=3，同机同盘，唯一变量=无）

解释器启动基线（`--help`，不碰账本）：**0.2705 / 0.2705 / 0.2788 s**。

| 段 | t0 | t1 | t2 |
|---|---|---|---|
| open 墙钟 | 14.512 s | 13.785 s | 14.400 s |
| `t_spawn → t_frame`（启动+血统装载+校验+写帧） | 0.470 | 0.423 | 0.439 |
| `t_frame → t_heads`（写血统头） | 0.029 | 0.023 | 0.028 |
| **`t_heads → t_release`（持锁做只读咨询）** | **14.307** | **13.392** | **14.355** |
| 争用方实际被阻塞 | 13.111 | 12.097 | 13.113 |

**post-write 占比 = 96.6% / 96.8% / 96.9%**（分母取 `t_release − t_spawn`，
把解释器启动也算进分母，故这是**低估**方向）。
换一种更保守的算法——把释放时刻上界钉在 open 进程退出时刻 `t_exit`
（真实释放必早于退出），得 **96.6% / 96.8% / 96.8%**。两种算法同值。

**独立交叉验证**：先前那篇 `open-latency-profile-v0.1` 用 cProfile 从**进程内部**得到
advisory 占 open 96.5%。本节用**文件系统 + 锁争用**从**进程外部**得到 96.6–96.9%。
两条方法学上不相干的路径给出同一个数。

### 9.3 由此得到的、比占比更锋利的一句

真正**需要**这把工作区级锁的，是"读血统 → 校验 → 写帧 → 写血统头"这一段：
`(t_spawn→t_heads) − 解释器启动` = **0.18 / 0.23 / 0.20 秒**。

**⇒ 锁被需要 ~0.2 秒，被持有 ~14.4 秒。约 1.4% 的必要性，换来 100% 的持有。**

### 9.4 本节的边界

1. **后半句没兑现，且不可追补。** 07-22 / 07-24 那几次真实超时当时谁持锁、持了多久，
   日志里没有，**历史不可重建**。本节证明的是"open 的持锁窗在结构上几乎全是 advisory"，
   不是"那几次超时是 advisory 造成的"。要坐实后者，只能在**未来**的争用当场记录持锁方
   （需要机件出可观测量，属改机制）。
2. **复制账本 ≠ 生产账本。** 副本与生产同机同盘、语料同源，但文件系统缓存态、
   碎片、并发背景都不必然相同。承重的是**占比**，不是 14.4 秒这个绝对值——
   冷/温摆幅已实测达 4.66x，大于任何代码版本差异，**绝对秒数不可跨轮比较**。
3. **`t_release` 有获取延迟。** 争用方比 open 进程退出晚 0.053–0.421 s 才拿到锁
   （`msvcrt.locking(LK_LOCK)` 的唤醒粒度）。这会**高估** post-write 窗；
   9.2 第二种算法就是为消掉这项而做的，两种算法同值说明这项不承重。
4. **n=3、单机、单一语料规模。** 未测占比随账本规模（帧数）的变化；
   结构上 advisory 是 O(帧数) 而写帧是 O(1)，故账本越大占比只会更高，但**这是推断，没测**。
5. **一行机件未改。** 本节的探针不 import 也不 patch 被测路径，只从外部看文件与锁。
6. 本节不改第七节任何一条，也不改我 2026-08-01 那一签给 Codex 的乙案范围。

### 9.5 复跑口径

```
# 1) 复制账本（robocopy 或等价），得到 <copy>
# 2) 取一个已闭合的头帧 id 作 --parent
python proposals/open-latency-lock-hold-v0.1/_probe_20260802_intra_lock_timeline.py \
    --ledger <copy> --parent <closed frame id> --trials 3 --warmups 1 \
    --out _probe_20260802_intra_lock_timeline.out.json
```

探针对生产账本只读；它拒绝以生产 `method-state` 为 `--ledger`（断言在 `main()` 里）。
一切 time 只当只追加文件内的身份键，不当时刻（`ledger-timestamp-authority-v0.1`）。
