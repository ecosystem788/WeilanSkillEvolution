...
CONCURRENT_RECEIPT_SCHEMA_VERSION = "weilan_concurrent_receipt_v0.1"
...
def append_jsonl_record_locked(path, record):
...
PROJECTION_SEMANTIC_DEPENDENCY_SCHEMA_VERSION = (
    "weilan_projection_semantic_dependency_v0.1"
)


def canonical_value_hash(value):
    encoded = json.dumps(
...
def semantic_projection_dependency_vector(workspace, scope, warnings=None):
    """Hash exactly the three branch-independent inputs used by the reducer."""

    warnings = warnings if warnings is not None else []
    all_entries = load_semantic_entries(workspace, scope, warnings)
    active_entries = active_semantic_entries(
        all_entries, workspace, scope, warnings
    )
    known_memory_ids = {entry.get("memory_id") for entry in all_entries}
    disposition_state = {
        memory_id: record.get("event_id")
        for memory_id, record in semantic_disposition_heads(
            workspace, scope, warnings
        ).items()
        if memory_id in known_memory_ids
    }
    components = {
        "active_semantic_entries": canonical_value_hash(active_entries),
        "semantic_dispositions": canonical_value_hash(disposition_state),
        "evidence_dependencies": canonical_value_hash(
            semantic_evidence_dependency_state(all_entries)
        ),
    }
    return {
        "schema_version": PROJECTION_SEMANTIC_DEPENDENCY_SCHEMA_VERSION,
        "components": components,
        "hash": canonical_value_hash(components),
    }
...
def command_prospective_observe(args):
    ...
    if not args.goal_ref:
        raise ValueError("prospective observation requires --goal-ref")
    warnings = []
    _, current_state = reduce_prospective(workspace, scope, warnings)
    if warnings or current_state["issues"]:
        raise ValueError("prospective ledger is invalid")
    goal = current_state["goals"].get(args.goal_ref)
    if not goal or goal.get("state") != "ACTIVE":
        raise ValueError("prospective observation requires a named active goal")
    observed = {
        "causal_event_id": str(uuid.uuid4()),
        "event_kind": args.kind,
        "event_name": args.name,
        "observed_at_utc": utc_now(),
        "registered_event_id": goal["registered_event_id"],
    ...
    if args.kind == "clock":
        matches = [
            goal_ref
            for goal_ref, candidate in current_state["goals"].items()
...
def mmr_rerank(results, lambda_, limit, tokens_key="_mmr_tokens"):
    """Select relevant but non-redundant results while preserving stable ties."""
    ...
    selected = []
    max_score = max((float(item.get("score", 0)) for item in remaining), default=1.0) or 1.0
    while remaining and len(selected) < limit:
        best_index = 0
        best_value = None
        for index, candidate in enumerate(remaining):
            candidate_tokens = set(candidate.get(tokens_key, []))
            redundancy = 0.0
            for chosen in selected:
                chosen_tokens = set(chosen.get(tokens_key, []))
            ...
            relevance = float(candidate.get("score", 0)) / max_score
...
def command_episode_search(args):
    ...
    for episode in index["episodes"]:
        ...
        if score:
            item = dict(episode)
            item["_mmr_tokens"] = list(episode.get("tokens", []))
    ...
    results = mmr_rerank(results, args.mmr_lambda, args.limit)
    for item in results:
...
def command_memory_search(args):
    ...
    for scope in sorted(set(scopes), key=str.casefold):
        ...
        for memory_id in candidate_ids:
            ...
            result = dict(entry)
            result["_mmr_tokens"] = sorted(entry_tokens)
            result["score"] = score
...
def active_lineage_branches(lineage_state):
    """The shared operational-branch predicate for rebuild and read preflight."""

    return {
        branch: state
        for branch, state in lineage_state["branches"].items()
        if state.get("status") == "active"
    }


def branch_selection_preflight(workspace, scope, warnings=None):
    warnings = warnings if warnings is not None else []
    lineage_state = derive_lineage_state(
        load_lineage_records(workspace, scope, warnings)
    )
    if lineage_state["issues"]:
        return {"branch_selection_needed": False}
    active_branches = active_lineage_branches(lineage_state)
    return {
        "branch_selection_needed": len(active_branches) > 1,
...
def command_projection_rebuild(args):
    ...
    projection = {
        ...
        "semantic_dependency_vector": semantic_projection_dependency_vector(
            workspace, scope, semantic_warnings
        ),
        "sources": sources,
        "source_snapshots": source_snapshots(sources, workspace),
        "control_heads": {
...
def projection_freshness(projection, controls):
    if not projection:
        return {
            ...
            "freshness": "stale",
            "adjudication": "clean",
            "branch_selection_needed": False,
            "reason_codes": ["projection_missing"],
        }
    if projection.get("schema_version") != PROJECTION_SCHEMA_VERSION:
        return {
            ...
            "adjudication": "clean",
            "branch_selection_needed": False,
            "reason_codes": ["legacy_projection_schema"],
        }
    ...
    semantic_warnings = []
    current_dependency_vector = semantic_projection_dependency_vector(
        workspace, scope, semantic_warnings
    )
    if projection.get("semantic_dependency_vector") != current_dependency_vector:
        reasons.append("semantic-stale")
    active_entries = active_semantic_entries(
        load_semantic_entries(workspace, scope, semantic_warnings),
        workspace,
        scope,
    ...
    adjudication = (
        "contested"
        if any(entry.get("contested_with") for entry in active_entries)
        else "clean"
    )
    advisory_reasons = ["contested"] if adjudication == "contested" else []
    fresh = not reasons
    return {
        "fresh": fresh,
        ...
        "branch_selection_needed": False,
        "reason_codes": reasons + advisory_reasons,
        "current_control_heads": current_heads,
        "current_semantic_dependency_vector": current_dependency_vector,
...
def evaluate_activation(workspace, scope, projection, controls):
    ...
    branch_preflight = branch_selection_preflight(workspace, scope)
    freshness.update(branch_preflight)
    if freshness["branch_selection_needed"]:
        freshness["reason_codes"].append("operational-stale-branch-needed")
...
def command_memory_recall(args):
    ...
    if selected:
        ...
        _, prospective_state = reduce_prospective(workspace, activation["scope"], warnings)
        open_agenda = [
            {
                key: goal.get(key)
                for key in (
            ...
            for goal in prospective_state["goals"].values()
...
def command_concurrent_receipt_append(args):
    if args.work_performed:
        raise ValueError(
            "concurrent sidecar receipts require work_performed=false; stop as blocker instead"
        )
    root = Path(args.root)
    ...
    record = {
        ...
        "open_error": args.open_error,
        "work_performed": False,
        "folded_by_frame_id": folded_by_frame_id,
        "source": args.source,
    }
    path = root / "concurrent-receipts.jsonl"
    append_jsonl_record_locked(path, record)
...
def build_parser():
    ...
    prospective_observe_parser.add_argument("--goal-ref", required=True)
    ...
    concurrent_receipt_parser = subparsers.add_parser(
        "concurrent-receipt-append",
        help="append one non-authoritative concurrent wake sidecar receipt",
    )
    concurrent_receipt_parser.add_argument("--root", required=True)
    concurrent_receipt_parser.add_argument("--wake-id", required=True)
    concurrent_receipt_parser.add_argument("--time")
    concurrent_receipt_parser.add_argument("--attempted-relation", choices=("continue", "fork"), required=True)
    concurrent_receipt_parser.add_argument("--attempted-parent", required=True)
    concurrent_receipt_parser.add_argument("--observed-head", required=True)
    concurrent_receipt_parser.add_argument("--open-error", required=True)
    concurrent_receipt_parser.add_argument("--source", required=True)
    concurrent_receipt_parser.add_argument("--folded-by-frame-id")
    concurrent_receipt_parser.add_argument("--work-performed", action="store_true")
    concurrent_receipt_parser.set_defaults(func=command_concurrent_receipt_append)
    ...
    memory_search_parser.add_argument("--mmr-lambda", type=float, default=1.0)
    ...
    episode_search_parser.add_argument("--mmr-lambda", type=float, default=1.0)
