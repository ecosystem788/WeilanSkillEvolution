...
from weilan_trace import mmr_rerank
...
def main():
    ...
    clustered = [
        {"id": "a1", "score": 4, "_mmr_tokens": ["memory", "shared"]},
        {"id": "a2", "score": 4, "_mmr_tokens": ["memory", "shared", "near"]},
        {"id": "b1", "score": 3, "_mmr_tokens": ["open", "question"]},
    ]
    if [item["id"] for item in mmr_rerank(clustered, 1.0, 2)] != ["a1", "a2"]:
        raise AssertionError("semantic MMR lambda=1 did not preserve legacy ordering")
    if [item["id"] for item in mmr_rerank(clustered, 0.2, 2)] != ["a1", "b1"]:
        raise AssertionError("semantic low-lambda MMR did not diversify clustered results")
    with tempfile.TemporaryDirectory(prefix="weilan-semantic-test-", dir=str(temp_parent)) as temporary:
        ...
        if activation["activation"]["state"] != "STALE":
            raise AssertionError("new semantic entry did not stale the cached projection")
        if "semantic-stale" not in activation["freshness"]["reason_codes"]:
            raise AssertionError("semantic append did not surface semantic-stale")
        run(
        ...
        if activation["activation"]["state"] != "ACTIVE":
            raise AssertionError("semantic rebuild did not restore active recall")
