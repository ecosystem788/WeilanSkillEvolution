...
def _cursor_observability(
    *,
    ...
    """Make "quiet wake" and "delta already consumed" distinguishable in-band.

    The load-bearing value is `cursor_before.updated_at_utc` -- the stamp this
    call READ, i.e. when the delta was last committed away.  A brief that only
    printed the stamp this call WROTE would show a fresh "just now" for both a
    genuinely quiet first wake and an accidental second run, which is exactly
    the ambiguity being reported on.  `cursor_commit` is listed separately so
    the reader can tell whether this call itself moved the cursor.

    Deliberately no `consumed_this_wake` boolean: without a stable wake-id
    contract there is no honest way to decide whether an earlier commit belongs
    to *this* wake.  These fields report; they do not adjudicate.

    They live on the `kind: "cursor"` source entry, not in `cursor_status`,
    because `site_fingerprint` hashes `cursor_status` whole but takes only
    `ref` from each source -- a per-call timestamp inside `cursor_status` would
    change the fingerprint on every wake and destroy its same-situation signal.
    """
    before: dict[str, Any] = {
        "file_exists": cursor_path.exists(),
        "loaded": cursor is not None,
        "updated_at_utc": cursor.get("updated_at_utc") if isinstance(cursor, dict) else None,
    }
    if load_reason:
        before["load_reason"] = load_reason
    return {
        "cursor_before": before,
        "cursor_commit": {
            "performed": bool(commit_cursor),
            "updated_at_utc": updated_at_utc if commit_cursor else None,
        },
    }
    ...
    brief = {
        ...
        "sources": [
            ...
            _source(
                ...
                "cursor",
                **_cursor_observability(
                    cursor_path=cursor_path,
                    cursor=cursor,
                    load_reason=cursor_reason,
...
def main(argv: list[str] | None = None) -> int:
    ...
    # The library has had commit_cursor=False since it was written, but no CLI
    # path reached it: reading the brief was unconditionally consuming.  --peek
    # only helps when it is used on the FIRST call of a wake; it cannot restore
    # a delta an earlier committing run already moved past.
    parser.add_argument(
        "--peek",
        "--no-commit-cursor",
        dest="peek",
        action="store_true",
        help="build the brief without committing the cursor; the same delta stays visible next run",
    )
