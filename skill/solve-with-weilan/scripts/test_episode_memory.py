...
def main():
    ...
    clustered = [
        {"id": "a1", "score": 3, "_mmr_tokens": ["alpha", "shared"]},
        {"id": "a2", "score": 3, "_mmr_tokens": ["alpha", "shared", "near"]},
        {"id": "b1", "score": 2, "_mmr_tokens": ["beta", "branch"]},
    ]
    if [item["id"] for item in mmr_rerank(clustered, 1.0, 2)] != ["a1", "a2"]:
        raise AssertionError("MMR lambda=1 did not preserve legacy ordering")
    if [item["id"] for item in mmr_rerank(clustered, 0.2, 2)] != ["a1", "b1"]:
        raise AssertionError("low-lambda MMR did not diversify clustered results")
    try:
        mmr_rerank(clustered, 1.1, 2)
    except ValueError:
        pass
    else:
        raise AssertionError("MMR accepted lambda outside [0,1]")
