...
```text
skill/solve-with-weilan/SKILL.md                 Main Codex Skill entrypoint / Codex Skill 主入口
skill/solve-with-weilan/agents/openai.yaml       Agent configuration / agent 配置
skill/solve-with-weilan/references/              Method contracts and subsystem specs / 方法契约与子系统说明
skill/solve-with-weilan/scripts/weilan_trace.py  Main CLI used by the agent / agent 使用的主 CLI
skill/solve-with-weilan/scripts/*.py             Runtime modules and tests / 运行模块与测试
theory/                                          WeiLan theory notes / 微澜理论资料
proposals/bounded-scheduler-v0.1/impl/            Bounded-autonomy scaffold source / 有界自治脚手架真源
```
...
git clone https://github.com/ecosystem788/WeilanSkillEvolution.git
Copy-Item -Recurse -Force ".\WeilanSkillEvolution\skill\solve-with-weilan" (Join-Path $skills "solve-with-weilan")
```
...
```powershell
cd .\WeilanSkillEvolution
git pull
Copy-Item -Recurse -Force ".\skill\solve-with-weilan" (Join-Path $skills "solve-with-weilan")
```
...
The repository-root `theory/` directory contains the source notes. These files preserve conceptual background and should be treated as theory provenance, not as automatic action authority.

仓库根目录的 `theory/` 保存微澜理论背景资料。它们是 theory provenance（理论来源依据），不是自动行动授权。
