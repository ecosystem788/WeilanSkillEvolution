...
Evidence: `proposals/se-0.4-se-0.7-program/SE-0.7_GATE_RECEIPT.md`, `evals/evolution/se-0.7-v0.1-manifest.json`, and `tests/test_bounded_evolution.py`. Loop mechanics exist and the v0.9 chain exercised proposal→shadow→adopt→deploy→canary once, but **repeated improvement on held-out evidence without guardrail degradation** has not been demonstrated — and cannot be demonstrated on the current near-ceiling suite (see SE-0.6 boundary (a)). SE-0.7 acceptance is therefore gated on the harder external suite (`fusion-dogfood-v0.1`) being approved and used for at least two successive successor evaluations.
...
1. **Critical path: external approval of `fusion-dogfood-v0.1`** (`proposals/fusion-dogfood-eval-cases/`).
   Pre-approval conditions are recorded in `proposals/fusion-dogfood-eval-cases/CLAUDE_REVIEW.md`:
   B1 copy-frozen fixtures with deliberately time-sliced snapshots (no live-path reads);
   B2 isolated environments for `fusion-memory-scope-recovery` and `fusion-scope-redirection-to-eval-proposal`;
   N1 same-hand clause (rubric freeze precedes judged candidates; scorer isolated from candidate author);
   N4 Method Impact Trace as a required receipt field.
...
3. Keep the deployed artifact unchanged until the harder suite exists; restrict the targeted deployment channel
   to non-method supporting changes and fold accumulated targeted changes into the next full-suite shadow.
