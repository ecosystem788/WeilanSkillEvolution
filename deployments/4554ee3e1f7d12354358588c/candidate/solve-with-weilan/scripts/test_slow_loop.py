...
- merge: lineage, source union (incl. evidence refs), grounding, net shrink;
...
def test_merged_entry_dies_with_terminal_evidence(state_home, tmp_path, capsys):
    source = tmp_path / "s.md"
    source.write_text("fact\n", encoding="utf-8")
    a = consolidate(capsys, "durable alpha decision", source)
    b = consolidate(capsys, "durable beta decision", source)

    # hand-plant an evidence record and give one input an evidence source by merging
    ...
    args = argparse.Namespace(
        ...
        source=["evidence:ev-merge-dep"], displace=[],
    )
    weilan_trace.command_memory_merge(args)
    merged = json.loads(capsys.readouterr().out)
